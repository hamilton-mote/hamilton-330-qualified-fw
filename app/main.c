...
#if defined(MODEL_330)
#define PROVIDED_FLAGS (0x3F)
#elif defined(MODEL_370)
...
uint8_t obuffer [sizeof(ham330_t)];
uint8_t iv [16];
...
void aes_populate(void) {
	cipher_encrypt_cbc(&aesc, iv, ((uint8_t*)&frontbuf) + AES_SKIP_START_BYTES,
					 sizeof(ham330_t)-AES_SKIP_START_BYTES, &obuffer[AES_SKIP_START_BYTES]);
